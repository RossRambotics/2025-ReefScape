    ...
    public void periodic() {
        ...
        if (llMeasurementBack != null && llMeasurementBack.tagCount > 0 && omegaRps < 2.0) {
            ...
            if (m_LastBackPose.getTranslation().getDistance(llMeasurementBack.pose.getTranslation()) > 1.0) {
                ...
                if (Math.abs(llMeasurementBack.pose.getRotation().getDegrees() - headingDeg) > 5.0) {
                    System.out.println("%%%%%%%%%%%%%%%%%%%%  Limelight Heading Error ll: "
                            + llMeasurementBack.pose.getRotation().getDegrees()
                            + " gyro: " + headingDeg + " %%%%%%%%%%%%%%%%%%%%%");
                    return;
                }
